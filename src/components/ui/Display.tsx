...
function Display(props: DisplayProps) {
  ...
  const students =
    typeof props.toShow.map === "function"
      ? props.toShow.map((el) => {
          return (
            <SCard
              data={el}
  ...
  useEffect(() => {
    //the infinite scroll part - changes no. elements displayed if scrolled to the end
  ...
  useEffect(() => {
    setPos(50);
  }, [props.toShow]); //reset pos to 50 if new search (i.e. if new props received)
