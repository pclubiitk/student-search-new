...
function check_bacchas(bacchas: string | string[], students: Student[]): Student[] {
...
function check_query(query: Query, students: Student[]): Student[] {
	...
	return students.filter((student: Student) => {
		...
		for (const key in query) {
			...
			if (Array.isArray(queryValue) && queryValue.length === 0) { //skip any fields that don't have anything in them
				continue;
			}
			...
			if (queryKey === "name") {
				...
				if (!test1) {
					...
					if (!(student.i.includes(lowercased_name)) && !(student.u.startsWith(lowercased_name))) {
						return false;
					}
				...
				const student_data = student.g.toLowerCase();
